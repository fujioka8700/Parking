...
| ------------ | -------- | ---------------- | ------------------------ |
| id           | Int      | PK, Auto         | 主キー                   |
| userId       | String   | UNIQUE, NOT NULL | ユーザーID               |
| password     | String   | NOT NULL         | パスワード（平文保存）   |
| createdAt    | DateTime | NOT NULL         | 作成日時                 |
| updatedAt    | DateTime | NOT NULL         | 更新日時                 |
...
- ユーザー認証機能を実装（ログイン・ログアウト機能）
- データの機密性を考慮した設計が必要
- 認証情報は環境に応じて異なる（開発環境と本番環境で異なるユーザーID/パスワード）
...
- セッション管理（Cookie ベース）
- 環境に応じた認証情報の設定
  - 開発環境: ユーザーID `user`、パスワード `password`
  - 本番環境: ユーザーID `osaka9999`、パスワード `4567`
